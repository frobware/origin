...
var _ = g.Describe("[sig-network-edge][Conformance][Area:Networking][Feature:Router]", func() {
	...
	var (
		...
		testNamespace = oc.KubeFramework().Namespace.Name
	)
	...
	g.AfterEach(func() {
		if g.CurrentGinkgoTestDescription().Failed {
			...
			if len(testNamespace) > 0 {
				output, _ := oc.Run("get").Args("all", "-o", "yaml", "-n", testNamespace).Output()
				e2e.Logf("resource state for namespace %q:\n%s\n", testNamespace, output)
			}
		}
	})
