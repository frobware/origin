...
func DoComputeEngineCreds(tc testpb.TestServiceClient, serviceAccount, oauthScope string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
	...
	if user != serviceAccount {
		return fmt.Errorf("Got user name %q, want %q.", user, serviceAccount)
	}
	if !strings.Contains(oauthScope, scope) {
		return fmt.Errorf("Got OAuth scope %q which is NOT a substring of %q.", scope, oauthScope)
	}
	return err
}
...
func DoServiceAccountCreds(tc testpb.TestServiceClient, serviceAccountKeyFile, oauthScope string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
	...
	if !strings.Contains(string(jsonKey), user) {
		return fmt.Errorf("Got user name %q which is NOT a substring of %q.", user, jsonKey)
	}
	if !strings.Contains(oauthScope, scope) {
		return fmt.Errorf("Got OAuth scope %q which is NOT a substring of %q.", scope, oauthScope)
	}
...
func DoJWTTokenCreds(tc testpb.TestServiceClient, serviceAccountKeyFile string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
...
func DoOauth2TokenCreds(tc testpb.TestServiceClient, serviceAccountKeyFile, oauthScope string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
...
func DoPerRPCCreds(tc testpb.TestServiceClient, serviceAccountKeyFile, oauthScope string) error {
	jsonKey, err := getServiceAccountJSONKey(serviceAccountKeyFile)
	if err != nil {
		return err
	}
	...
	token, err := GetToken(serviceAccountKeyFile, oauthScope)
	if err != nil {
		return err
	}
...
func DoGoogleDefaultCredentials(tc testpb.TestServiceClient, defaultServiceAccount string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
	...
	if reply.GetUsername() != defaultServiceAccount {
		return fmt.Errorf("Got user name %q; wanted %q. ", reply.GetUsername(), defaultServiceAccount)
	}
	return nil
}

// DoComputeEngineChannelCredentials performs an unary RPC with compute engine channel credentials
func DoComputeEngineChannelCredentials(tc testpb.TestServiceClient, defaultServiceAccount string) error {
	pl, err := ClientNewPayload(testpb.PayloadType_COMPRESSABLE, largeReqSize)
	if err != nil {
		return err
	}
...
func DoPickFirstUnary(tc testpb.TestServiceClient) error {
	const rpcCount = 100
	...
	for i := 0; i < rpcCount; i++ {
		...
		if err != nil {
			return fmt.Errorf("iteration %d, failed to do UnaryCall: %v", i, err)
		}
		id := resp.ServerId
		if id == "" {
			return fmt.Errorf("iteration %d, got empty server ID", i)
		}
		if i == 0 {
			serverID = id
			continue
		...
		if serverID != id {
			return fmt.Errorf("iteration %d, got different server ids: %q vs %q", i, serverID, id)
		}
	}
	return err
}
